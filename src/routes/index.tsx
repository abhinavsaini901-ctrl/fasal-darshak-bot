...
function HomePage() {
  return (
    ...
    <PageShell>
      ...
      {/* AI खेती गुरु — PREMIUM AI FARMING ASSISTANT */}
      <section className="relative overflow-hidden bg-gradient-to-br from-success/10 via-background to-primary/10 py-12">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_30%,var(--color-success)_0%,transparent_45%)] opacity-10" />
        <div className="relative mx-auto max-w-6xl px-4">
          ...
          <div className="mb-8 text-center">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-success/15 px-3 py-1 text-xs font-bold text-success">
              <Sparkles className="h-3 w-3" /> AI पावर्ड
            </span>
            <h2 className="mt-3 text-3xl font-bold text-foreground md:text-4xl lg:text-5xl">
              🌾 AI <span className="text-success">खेती गुरु</span>
            </h2>
            <p className="mx-auto mt-2 max-w-2xl text-sm text-muted-foreground md:text-base">
              कैमरा खोलें, फसल या पेड़ दिखाएं और तुरंत AI से जानकारी प्राप्त करें
            </p>
          </div>

          {/* Main glassmorphism card */}
          <Card className="relative overflow-hidden border border-success/20 bg-gradient-to-br from-card/90 via-card/80 to-card/90 shadow-strong backdrop-blur-xl">
            <div className="grid items-center gap-8 p-6 md:p-10 lg:grid-cols-[1fr_auto_1fr]">
              {/* Left features */}
              <div className="grid grid-cols-2 gap-3">
                {[
                  { icon: Camera, title: "Live Crop Scanner", desc: "फसल लाइव स्कैन" },
                  { icon: TreePine, title: "Tree Identification", desc: "पेड़ पहचान" },
                  { icon: Microscope, title: "Disease Detection", desc: "रोग पहचान" },
                  { icon: Bug, title: "Pest Detection", desc: "कीट पहचान" },
                  { icon: FlaskConical, title: "Nutrient Analysis", desc: "पोषक तत्व" },
                ].map((f) => (
                  <div
                    key={f.title}
                    className="flex flex-col items-start gap-2 rounded-2xl border border-border/60 bg-secondary/40 p-3 transition-all hover:-translate-y-0.5 hover:border-success/30 hover:bg-success/5"
                  >
                    ...
                    <p className="text-[11px] leading-tight text-muted-foreground">{f.desc}</p>
                  </div>
                ))}
              </div>

              {/* Center camera with glow */}
              <div className="relative flex flex-col items-center justify-center py-4">
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="h-56 w-56 rounded-full bg-success/20 blur-3xl" />
                </div>
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="absolute h-48 w-48 animate-pulse rounded-full border border-success/20" />
                  <div className="absolute h-40 w-40 animate-pulse rounded-full border border-primary/20 [animation-delay:0.5s]" />
                </div>
                <Link to="/scanner" className="relative z-10">
                  <div className="flex h-36 w-36 items-center justify-center rounded-full bg-gradient-to-br from-success to-primary shadow-strong transition-transform hover:scale-105 md:h-44 md:w-44">
                    <Camera className="h-16 w-16 text-primary-foreground md:h-20 md:w-20" />
                  </div>
                </Link>
                <p className="relative z-10 mt-4 text-center text-sm font-bold text-foreground">
                  Camera Scan Now
                </p>
                <p className="relative z-10 text-xs text-muted-foreground">AI से तुरंत पहचान</p>
              </div>

              {/* Right features */}
              <div className="grid grid-cols-2 gap-3">
                {[
                  { icon: Mic, title: "Voice Assistant", desc: "हिंदी आवाज़" },
                  { icon: MessageCircle, title: "Ask AI Guru", desc: "AI से पूछें" },
                  { icon: CloudSun, title: "Weather Tips", desc: "मौसम सलाह" },
                  { icon: TrendingUp, title: "Yield Prediction", desc: "उपज अनुमान" },
                  { icon: IndianRupee, title: "Cost & Profit", desc: "लागत और मुनाफा" },
                ].map((f) => (
                  <div
                    key={f.title}
                    className="flex flex-col items-start gap-2 rounded-2xl border border-border/60 bg-secondary/40 p-3 transition-all hover:-translate-y-0.5 hover:border-success/30 hover:bg-success/5"
            ...
            <div className="flex flex-wrap items-center justify-center gap-3 border-t border-border/60 bg-secondary/30 p-5">
              <Link to="/scanner">
                <Button size="lg" className="h-12 rounded-xl bg-gradient-primary px-6 text-base font-bold shadow-soft">
                  <Camera className="mr-2 h-5 w-5" /> Camera Scan Now
                </Button>
              </Link>
              <Link to="/scanner">
                <Button size="lg" variant="outline" className="h-12 rounded-xl border-success/30 px-6 text-base font-semibold text-success hover:bg-success/10">
                  <MessageCircle className="mr-2 h-5 w-5" /> Ask AI Guru
                </Button>
              </Link>
              <Link to="/scanner">
                <Button size="lg" variant="secondary" className="h-12 rounded-xl px-6 text-base font-semibold">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { label: "Crop Name", value: "धान (Rice)" },
              { label: "Disease Name", value: "ब्लास्ट रोग" },
              { label: "Severity Level", value: "मध्यम (60%)" },
              { label: "Treatment", value: "ट्राइसाइक्लाजॉल स्प्रे" },
              { label: "Prevention", value: "संतुलित नाइट्रोजन" },
              { label: "Fertilizer", value: "NPK 20:20:20" },
              { label: "Water Requirement", value: "हल्की सिंचाई" },
              { label: "Harvest Time", value: "120-130 दिन" },
            ].map((item) => (
              <div
                key={item.label}
          ...
          <div className="mt-6 flex flex-wrap items-center justify-center gap-2">
            {["Gemini Vision API", "Speech-to-Text", "Text-to-Speech", "Supabase Backend", "Real-time AI Chat"].map((tech) => (
              <span
                key={tech}
